...
function SmartAI:useCardIronChain(card, use)
	...
	for _, friend in ipairs(self.friends) do
		...
		if friend:isChained() and not self:isGoodChainPartner(friend) and self:hasTrickEffective(card, friend) and not friend:hasSkill("danlao")
			and not (friend:getMark("@duilieB") > 0 and math.mod(card:getNumber(), 2) == 0 and card:getNumber() > 0) then
...
function SmartAI:useCardFireAttack(fire_attack, use)
	...
	local can_attack = function(enemy)
		...
		if enemy:getMark("@duilieB") > 0 and math.mod(fire_attack:getNumber(), 2) == 0 and fire_attack:getNumber() > 0 then return false end
		
		if enemy:getEquips():isEmpty() then
