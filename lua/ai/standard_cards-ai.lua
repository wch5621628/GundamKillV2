...
function SmartAI:slashIsEffective(slash, to, from, ignore_armor)
	...
	if to:getMark("@duilieB") > 0 and math.mod(slash:getNumber(), 2) == 0 and slash:getNumber() > 0 then return false end

	if to:hasSkill("qingzhuang") and blankEquipArea(to) and slash:isRed() then return false end

	if (slash:isKindOf("ThunderSlash") or slash:isKindOf("FireSlash")) and to:getEquips():isEmpty() then
	...
	if to:hasSkill("jinduan") and slash:isRed() and self:isEnemy(to) and #self.friends_noself > 0 then return false end

	if to:hasSkill("tiebi") and (slash:isRed() or slash:isKindOf("FireSlash")) and to:distanceTo(from) > 1 then return false end

	if to:hasSkill("bachi") and slash:isRed() and self:isEnemy(to) and (not to:isNude()) then return false end
...
function SmartAI:isPriorFriendOfSlash(friend, card, source)
	...
	if friend:hasSkill("jinduan") and card:isRed() then return true end
	if friend:hasSkill("bachi") and card:isRed() and friend:getCardCount() > 2 and #self.enemies > 1 then return true end
	return
end
...
function SmartAI:useCardPeach(card, use)
	...
	if self.player:getMark("@duilieB") > 0 and math.mod(card:getNumber(), 2) == 0 and card:getNumber() > 0 then return false end
...
function SmartAI:useCardDuel(duel, use)
	...
	local canUseDuelTo=function(target)
		if target:hasSkill("mingren") then return false end
...
function SmartAI:useCardSnatchOrDismantlement(card, use)
	...
	local addTarget = function(player, cardid)
		if not table.contains(targets, player:objectName())
			...
			and not (player:getMark("@duilieB") > 0 and math.mod(sgs.Sanguosha:getCard(cardid):getNumber(), 2) == 0 and sgs.Sanguosha:getCard(cardid):getNumber() > 0)
			then
			if not usecard then
				use.card = card
				usecard = true
...
function SmartAI:useCardCollateral(card, use)
	...
	for _, enemy in ipairs(fromList) do
		if (not use.current_targets or not table.contains(use.current_targets, enemy:objectName()))
			...
			and enemy:getWeapon() 
			and not (enemy:getMark("@duilieB") > 0 and math.mod(card:getNumber(), 2) == 0 and card:getNumber() > 0) then
...
wooden_ox_skill.getTurnUseCard = function(self)
	...
	if card and friend and friend:objectName() ~= self.player:objectName() and (self:getOverflow() > 0 or self:isWeak(friend))
		and hasEquipArea(friend, "treasure") then
