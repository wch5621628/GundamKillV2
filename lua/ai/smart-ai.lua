...
function SmartAI:adjustKeepValue(card, v)
	...
	if self.player:getPile("&hubi"):contains(card:getEffectiveId()) then
        v = v - 0.1
    end
	...
	if self.player:hasSkill("ronghe") then
		...
		if #list > 0 then
			for _,l in ipairs(list) do
				local id = tonumber(l)
				if id and id == card:getEffectiveId() then
					v = v + 1
				end
			end
		end
...
function SmartAI:getKnownNum(player)
    ...
    if not player then
		...
		if player:hasSkill("ronghe") then
		...
		for _, id in sgs.qlist(player:getPile("&hubi")) do
            cards:append(sgs.Sanguosha:getCard(id))
        end
...
function SmartAI:getCardId(class_name, player, acard)
    ...
    for _, card in ipairs(cards) do
        ...
        local isCard = card:isKindOf(class_name) and not prohibitUseDirectly(card, self.player)
                        and (card_place ~= sgs.Player_PlaceSpecial or self.player:getPile("wooden_ox"):contains(card:getEffectiveId()) or self.player:getPile("&hubi"):contains(card:getEffectiveId()))
        if viewas then
...
function SmartAI:getCards(class_name, flag)
    ...
    elseif flag:match("h") then
		...
		for _, id in sgs.qlist(self.player:getPile("&hubi")) do
            all_cards:append(sgs.Sanguosha:getCard(id))
        end
    end
